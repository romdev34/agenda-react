...
export default function Header() {
    ...
    const handleDisableButton = function() {
        if (agendaType === "monthly") {
            return monthReducer.disableButton
        }
        if (agendaType === "weekly") {
            return weekReducer.disableButton
        }
        if (agendaType === "daily") {
            return  dayReducer.disableButton
        }
    }
    ...
    const displayYear = function () {
        if (agendaType === "weekly") {
            return weekReducer.year
        }
        if (agendaType === "monthly") {
            return new Date(year, monthNumber).getFullYear()
        }
    }
        ...
        return (
            <>
                ...
                <div
                    className="ml-auto items-center flex justify-end select-none">
                    ...
                    <ButtonToday disableButton={handleDisableButton()}/>
                    <div className="p-2">
                <span onClick={handleToPrevious}
                      className="cursor-pointer text-3xl">&#9664;</span>
                        <span
                            className="inline-block w-[230px] capitalize text-xl">{`${agendaType === "monthly" ? month : agendaType === "weekly" ? " semaine " + weekNumber : dayName + date}`}</span>
